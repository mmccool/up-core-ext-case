...
camera_bx = 28;
camera_by = 28;
camera_br = m2_hole_radius + 0.2; // M2
camera_nr = m2_nut_radius + 0.4;
camera_rr = 2;
camera_sx = 20;
camera_sy = 20;
camera_sr = 5/2;
  ...
  translate([0,0,case_z+standoff_h-case_p]) {
    difference() {
      union() {
        ...
        translate([0,0,case_p-standoff_h+1.5*sleeve_u-1]) 
          rotate([180,0,0])
            rcube(case_x-3*case_r+2*(case_r-case_t-sleeve_tol)-1-18,
                  case_y-2*case_r+2*(case_r-case_t-sleeve_tol)-1,
                  1.3*sleeve_u,0.5,case_sm);
      }
      // cutout for ring
      translate([0,0,sleeve_z-sleeve_u-ring_h-standoff_h+case_p]) 
        ring(r_tol=tol);
      // wiring holes for ring, including taper for solder clearance
      for (i=[0:15]) {
        rotate((i-0.5)*360/16) 
          translate([ring_R-2,0,sleeve_z-sleeve_u-ring_h-standoff_h+case_p-10]) {
            cylinder(r=1,h=20,$fn=4*sm_base);
            translate([0,0,10-1+tol]) cylinder(r1=1,r2=2,h=1,$fn=4*sm_base);
          }
      }
      // cutout for sticks
      hull() {
      ...
      difference() {
        hull() {
          translate([0,-insert_y/2,-standoff_h+case_p+sleeve_z-sleeve_u-stick_z-tol]) 
            stick(x_tol=10,y_tol=tol,z_tol=tol);
          translate([0,-insert_y/2-stick_y-tol,
                     -standoff_h+case_p+sleeve_z-sleeve_u-stick_z-tol]) 
            stick(x_tol=10,y_tol=tol,z_tol=stick_y);
        }
        translate([case_x/2-power_ix,
                     -case_y/2+case_r-(case_r-case_t-sleeve_tol+0),
                     case_p-standoff_h+sleeve_u+tol+power_iy]) {
          rotate([-90,0,0]) 
            cylinder(r1=0.95*power_R,r2=power_R+0.5,h=5,$fn=power_sm);
      ...
      hull() {
        translate([insert_x/2,0,-standoff_h+case_p+sleeve_z-sleeve_u-stick_z-tol]) 
          rotate(90) 
            stick(x_tol=10,y_tol=tol,z_tol=tol);
        translate([insert_x/2+stick_y+tol,0,
                   -standoff_h+case_p+sleeve_z-sleeve_u-stick_z-tol]) 
          rotate(90) 
            stick(x_tol=10,y_tol=tol,z_tol=stick_y);
      }
      ...
      translate([-camera_x/2+camera_ie,-camera_y/2+camera_ie,
                 sleeve_z-sleeve_u-camera_ch])
      ...
      translate([-camera_x/2+camera_ie,-camera_y/2+camera_ie,
                 sleeve_z-sleeve_u-camera_ch+insert_sh+1.5])
      ...
      hull() {
        translate([-camera_bx/2+camera_nr+2*camera_br,-camera_by/2,
                   sleeve_z-sleeve_u-camera_ch])
          cylinder(r=1.3*camera_br,h=0.3*camera_ch,$fn=camera_sm);
        translate([ camera_bx/2-camera_nr-2*camera_br,-camera_by/2,
                   sleeve_z-sleeve_u-camera_ch])
          cylinder(r=1.3*camera_br,h=0.3*camera_ch,$fn=camera_sm);
        translate([-camera_bx/2+camera_nr+2*camera_br,-camera_by/2+1,
                   sleeve_z-sleeve_u-camera_ch])
          cylinder(r=1.3*camera_br,h=0.3*camera_ch,$fn=camera_sm);
        translate([ camera_bx/2-camera_nr-2*camera_br,-camera_by/2+1,
                   sleeve_z-sleeve_u-camera_ch])
      ...
      hull() {
        translate([-camera_bx/2,-camera_by/2+camera_nr+2*camera_br,
                   sleeve_z-sleeve_u-camera_ch])
          cylinder(r=1.3*camera_br,h=0.3*camera_ch,$fn=camera_sm);
        translate([-camera_bx/2, camera_by/2-camera_nr-2*camera_br,
                   sleeve_z-sleeve_u-camera_ch])
          cylinder(r=1.3*camera_br,h=0.3*camera_ch,$fn=camera_sm);
        translate([-camera_bx/2+1,-camera_by/2+camera_nr+2*camera_br,
                   sleeve_z-sleeve_u-camera_ch])
          cylinder(r=1.3*camera_br,h=0.3*camera_ch,$fn=camera_sm);
        translate([-camera_bx/2+1, camera_by/2-camera_nr-2*camera_br,
                   sleeve_z-sleeve_u-camera_ch])
      ...
      translate([case_x/2-power_ix,0,case_p-standoff_h+sleeve_u+tol+power_iy]) {
        ...
        translate([-0.75*2*power_r/2,-100,-2*power_r]) 
          cube([0.75*2*power_r,100,2*power_r]);
      }
      translate([case_x/2-power_ix,
                 -case_y/2+case_r-(case_r-case_t-sleeve_tol),
                 case_p-standoff_h+sleeve_u+tol+power_iy]) {
        rotate([90,0,0]) 
          cylinder(r=power_R,h=100,$fn=power_sm);
        translate([0,3.5,0]) rotate([-90,0,0]) 
          cylinder(r=power_nr,h=100,$fn=power_sm);
      }
      // power daisy chain
      translate([case_x/2-power_ox,0,case_p-standoff_h+tol+power_h]) {
        translate([-power_w/2,-100,-10]) 
          cube([power_w,100,10]);
      }
      ...
      hull() {
        ...
        translate([-case_x/2+bolt_ox,case_y/2-bolt_oy-3,-5+12])
          sphere(r=case_sbr,$fn=sleeve_sm);
        translate([bolt_x-case_x/2+bolt_ox,case_y/2-bolt_oy-3,-5])
          cylinder(r=case_sbr,h=10,$fn=sleeve_sm);
        translate([bolt_x-case_x/2+bolt_ox,case_y/2-bolt_oy-3,-5+12])
          sphere(r=case_sbr,$fn=sleeve_sm);
      }
    }
  }
...
module sleeve() {
  ...
  difference() {
    ...
    // hole for power jack
    translate([case_x/2-power_ix,2,case_z+sleeve_u+tol+power_iy]) {
      rotate([90,0,0]) 
        cylinder(r=power_R,h=100,$fn=power_sm);
      translate([-power_R,-100,-2*power_R]) 
        cube([2*power_R,100,2*power_R]);
    }
    // power daisy chain
    translate([case_x/2-power_ox,0,case_z+tol+power_h]) {
      translate([-power_w/2,-100,-10]) 
        cube([power_w,100,10]);
    }
  }
